...
3. **AI Relevance Filter (Haiku)** — Chain-of-thought: Haiku writes analysis BEFORE scoring (analysis field precedes score in JSON). Legal-specific rubric: contrary authority scores 8-10 (lawyers must know opposing holdings). Strict filtering: mere keyword overlap with unrelated legal areas scores 1-2. Court hierarchy preference (SC > HC > Tribunal). Provisions data included in candidate info. Keeps score >= 6 (min_score default). Haiku max_tokens=4000, timeout=60s.
