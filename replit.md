...
3. **AI Relevance Filter (Opus 4.6 with Adaptive Thinking)** — Uses `claude-opus-4-6` with `thinking={"type": "adaptive"}` for deep legal reasoning on relevance. Chain-of-thought: Opus writes analysis BEFORE scoring (analysis field precedes score in JSON). Response parsing extracts text block (skipping thinking blocks). Legal-specific rubric: contrary authority scores 8-10 (lawyers must know opposing holdings). Strict filtering: mere keyword overlap with unrelated legal areas scores 1-2. Court hierarchy preference (SC > HC > Tribunal). Provisions data included in candidate info. Keeps score >= 6 (min_score default, synced in both prompt and code). Opus max_tokens=8000, timeout=120s, 1 retry on JSON parse failure. Internal fields (usage, timing_ms) stripped from expanded_query in all response paths (both GET and POST). Filter step takes ~60-90s due to adaptive thinking depth.
